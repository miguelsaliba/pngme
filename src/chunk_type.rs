...
impl fmt::Display for ParseChunkTypeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Error formatting chunk")
    }
}
...
impl FromStr for ChunkType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.len() != 4 {
            return Err(Box::new(ParseChunkTypeError {}));
        }
...
impl TryFrom<[u8; 4]> for ChunkType {
    ...
    fn try_from(args: [u8; 4]) -> Result<Self, Self::Error> {
        if check_bytes(&args) {
            Ok(ChunkType {
                bytes: args.clone(),
            })
...
fn check_bytes(bytes: &[u8; 4]) -> bool {
    for &item in bytes {
        if !(item.is_ascii_uppercase() || item.is_ascii_lowercase()) {
            return false;
        }
    }
    true
